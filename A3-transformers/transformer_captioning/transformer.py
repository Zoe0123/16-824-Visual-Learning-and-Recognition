...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        denom = torch.sqrt(self.embed_dim)
        dot_product = torch.bmm(query, key.transpose(1, 2))/denom
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = self.query_proj(key).view(N, T, H, D//H).transpose(1, 2)
        value = self.query_proj(value).view(N, T, H, D//H).transpose(1, 2)
        ...
        denom = torch.sqrt(self.embed_dim)
        dot_product = torch.matmul(query, key.transpose(2, 3))/denom
...
class SelfAttentionBlock(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dropout=0.1):
        ...
        self.dropout = nn.Dropout(dropout)
        self.layernorm = nn.LayerNorm(input_dim, eps=5e-08) # only change eps when case 3
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dropout=0.1):
        ...
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.LayerNorm(input_dim, eps=1e-06) # only change eps when case 3
