...
class DiffusionModel(nn.Module):
    ...
    def get_posterior_parameters(self, x_0, x_t, t):
        ...
        shape = x_t.shape
        coef2, coef1 = extract(self.posterior_mean_coef2, t, shape), extract(self.posterior_mean_coef1, t, x_0.shape)
        posterior_mean = coef2 * x_t + coef1 * x_0
        posterior_variance = extract(self.posterior_variance, t, shape)
        posterior_log_variance_clipped = extract(self.posterior_log_variance_clipped, t, shape)
    ...
    def model_predictions(self, x_t, t):
        ...
        coeff1 = extract(self.x_0_pred_coef_1, t, x_t.shape)
        coeff2 = extract(self.x_0_pred_coef_2, t, pred_noise.shape)
        x_0 = (coeff1 * x_t + coeff2 * pred_noise).clamp(min=-1.0, max=1.0)
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        # N = x.shape[0]
        # t_lst = torch.full((N,), t.item(), dtype=torch.long, device=self.device)
        _, x_0 = self.model_predictions(x,t)
        posterior_mean, posterior_variance, _ = self.get_posterior_parameters(x_0,x,t)
        # Sample a noise vector z if t > 0 otherwise, z = 0
        if t > 0:
            z = torch.randn(x.shape, device=self.device)
        else:
            z = 0
        pred_img = posterior_mean + torch.exp(posterior_variance)*z
    ...
    def sample_times(self, total_timesteps, sampling_timesteps):
        ...
        return torch.linspace(-1, total_timesteps-1, steps = sampling_timesteps+1)
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        tau_i_lst = torch.full((batch,), tau_i, device=device, dtype=torch.long)
        pred_noise, x_0 = model_predictions(img,tau_i_lst)
        ...
        sigma_i = eta * torch.sqrt((1 - alpha_i / alpha_isub1) * (1 - alpha_isub1) / (1 - alpha_i))
        # compute the coefficient of \epsilon_{\tau_{i}}
        coef_eps = torch.sqrt(1-alpha_isub1-sigma_i**2)
        # sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: use the reparameterization trick
        if tau_i > 0:
            z = torch.randn(img.shape, device=self.device)
        else:
            z = 0
        img = x_0*torch.sqrt(alpha_isub1) + coef_eps * pred_noise + sigma_i*z
