...
def get_optimizers_and_schedulers(gen, disc):
    ...
    scheduler_discriminator = torch.optim.lr_scheduler.LinearLR(optim_discriminator, start_factor=1., end_factor=1/5e5, total_iters=5e5)
    scheduler_generator = torch.optim.lr_scheduler.LinearLR(optim_generator, start_factor=1., end_factor=1/1e5, total_iters=1e5)
...
def train_model(
    ...
    train_loader = torch.utils.data.DataLoader(
        Dataset(root="/mnt/data/datasets/CUB_200_2011_32", transform=ds_transforms),
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                B = len(train_batch)
                gen_out = gen(B)
                fake_out = disc(gen_out)

                real_out = disc(train_batch)

                # TODO: 1.5 Compute the interpolated batch and run the discriminator on it.
                a = torch.rand(1).cuda()
                interp_data = a * gen_out + (1 - a) * train_batch
                interp_out = disc(interp_data)
                discriminator_loss = disc_loss_fn(
                    real_out, fake_out, interp_out, interp_data, lamb)
            ...
            if iters % 5 == 0:
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    ...
                    gen_out = gen(B)
                    fake_out = disc(gen_out)
                    generator_loss = gen_loss_fn(fake_out)
