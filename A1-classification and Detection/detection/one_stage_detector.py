...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.dummy_out_shapes = [(key, value.shape) for key, value in dummy_out.items()]
        ...
        self.fpn_params["p3_c1"] = nn.Conv2d(self.dummy_out_shapes[0][1][1], self.out_channels, 1, stride=1, padding=0)
        self.fpn_params["p4_c1"] = nn.Conv2d(self.dummy_out_shapes[1][1][1], self.out_channels, 1, stride=1, padding=0)
        self.fpn_params["p5_c1"] = nn.Conv2d(self.dummy_out_shapes[2][1][1], self.out_channels, 1, stride=1, padding=0)

        self.fpn_params["p3_c3"] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params["p4_c3"] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        self.fpn_params["p5_c3"] = nn.Conv2d(self.out_channels, self.out_channels, 3, stride=1, padding=1)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        feats5 = self.fpn_params["p5_c1"](backbone_feats["c5"])
        fpn_feats["p5"] = self.fpn_params["p5_c3"](feats5)

        feats4 =self.fpn_params["p4_c1"](backbone_feats["c4"])
        feats4 = feats4 + F.interpolate(feats5, scale_factor=2, mode='nearest')
        fpn_feats["p4"] = self.fpn_params["p4_c3"](feats4)

        feats3 = self.fpn_params["p3_c1"](backbone_feats["c3"])
        feats3 = feats3 + F.interpolate(feats4, scale_factor=2, mode='nearest')
        fpn_feats["p3"] = self.fpn_params["p3_c3"](feats3)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Fill these.
        stem_cls = [nn.Conv2d(in_channels, stem_channels[0], 3, stride=1, padding=1), nn.ReLU()]
        stem_box = [nn.Conv2d(in_channels, stem_channels[0], 3, stride=1, padding=1), nn.ReLU()]
        # Replace "pass" statement with your code
        for i in range(len(stem_channels)-1):
            stem_cls.extend([nn.Conv2d(stem_channels[i], stem_channels[i+1], 3, stride=1, padding=1), nn.ReLU()])
            stem_box.extend([nn.Conv2d(stem_channels[i], stem_channels[i+1], 3, stride=1, padding=1), nn.ReLU()])
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        p3_class = self.pred_cls(self.stem_cls(feats_per_fpn_level["p3"]))
        B, C, _, _ = p3_class.shape
        p3_class = p3_class.view(B, -1, C)
        class_logits = {"p3": p3_class,
        "p4": self.pred_cls(self.stem_cls(feats_per_fpn_level["p4"])).view(B, -1, C),
        "p5": self.pred_cls(self.stem_cls(feats_per_fpn_level["p5"])).view(B, -1, C)}
        boxreg_deltas = {"p3": self.pred_box(self.stem_box(feats_per_fpn_level["p3"])).view(B, -1, 4),
        "p4": self.pred_box(self.stem_box(feats_per_fpn_level["p4"])).view(B, -1, 4),
        "p5": self.pred_box(self.stem_box(feats_per_fpn_level["p5"])).view(B, -1, 4)}
        centerness_logits = {"p3": self.pred_ctr(self.stem_box(feats_per_fpn_level["p3"])).view(B, -1, 1),
        "p4": self.pred_ctr(self.stem_box(feats_per_fpn_level["p4"])).view(B, -1, 1),
        "p5": self.pred_ctr(self.stem_box(feats_per_fpn_level["p5"])).view(B, -1, 1)}
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        pred_boxreg_deltas, matched_gt_deltas = pred_boxreg_deltas.view(-1, 4), matched_gt_deltas.view(-1, 4)
        pred_cls_logits, matched_gt_boxes = pred_cls_logits.view(-1, 20), matched_gt_boxes.view(-1, 5)
        pred_ctr_logits = pred_ctr_logits.view(-1)

        # bg_inds = torch.argwhere(matched_gt_deltas, matched_gt_deltas==[-1, -1, -1, -1])[0]
        bg_inds = (matched_gt_boxes[:, 4] == -1)
        fg_inds = (matched_gt_boxes[:, 4] != -1)
        ...
        loss_ctr[bg_inds] *= 0.0

        # matched_gt_cls = matched_gt_boxes[:,-1].squeeze().to(torch.int32)
        # matched_gt_cls[bg_inds] = 0
        # gt_cls = nn.functional.one_hot(matched_gt_cls, num_classes=self.num_classes)
        matched_gt_cls = matched_gt_boxes[:,-1]
        gt_cls = torch.zeros_like(pred_cls_logits)
        gt_cls[fg_inds] = torch.scatter(gt_cls[fg_inds], index=matched_gt_cls[fg_inds,None].long().expand(gt_cls[fg_inds].shape), src=torch.ones_like(gt_cls[fg_inds]), dim=-1)
        loss_cls = sigmoid_focal_loss(pred_cls_logits, gt_cls)
        loss_ctr[bg_inds] *= 0.0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_classes = torch.argmax(level_pred_scores, dim=-1).cuda()
            level_pred_scores = torch.take_along_dim(level_pred_scores,level_pred_classes[:,None], dim=-1).squeeze().cuda()
            # Step 1:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, self.backbone.fpn_strides[level_name]).cuda()

            
            # Step 2:
            # Replace "pass" statement with your code
            inds = (level_pred_scores > test_score_thresh)

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_classes = level_pred_classes[inds]
            level_pred_scores = level_pred_scores[inds]
            level_pred_boxes = level_pred_boxes[inds]

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            H, W = images[0].shape[-1], images[0].shape[-2]
            level_pred_boxes[:,0] = torch.clip(level_pred_boxes[:,0], min=0, max=H-1)
            level_pred_boxes[:,1] = torch.clip(level_pred_boxes[:,0], min=0, max=H-1)
            level_pred_boxes[:,2] = torch.clip(level_pred_boxes[:,0], min=0, max=W-1)
            level_pred_boxes[:,3] = torch.clip(level_pred_boxes[:,0], min=0, max=W-1)
