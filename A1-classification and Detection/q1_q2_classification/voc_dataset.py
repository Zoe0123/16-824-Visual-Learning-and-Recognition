...
class VOCDataset(Dataset):
    ...
    def __init__(self, split, size, data_dir='/mnt/data/data/VOCdevkit/VOC2007/'): #change back
        super().__init__()
        self.split = split
                ...
                if child.tag == 'object':
                    for grandchild in child:
                        if grandchild.tag == 'name':
                            ind = self.get_class_index(grandchild.text)
                            class_vec[ind] = 1
    ...
    def get_random_augmentations(self):
        ...
        #     transforms.CenterCrop(20), #?? todo: `flat_dim` in line 46 in simple_cnn.py
        #     transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)), # todo: adjust
        # )
        # test
        if self.split == 'test':
        ...
        # train
            ...
            transform = [transforms.RandomCrop(self.size), transforms.RandomHorizontalFlip(p=0.5), transforms.RandomVerticalFlip(p=0.5)]

            #  transforms.ColorJitter(brightness=0.5, contrast=1, saturation=0.1, hue=0.5), transforms.GaussianBlur(3),  transforms.RandomRotation(10)
        return transform
